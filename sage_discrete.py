"""
SAGE Evaluation for discrete data for experiments in 'Causal Structure Learning for Efficient SAGE Estimation'

Command line args:
    --data csv-file in folder ~/data/ (string without suffix)
    --model choice between naive Bayes classifier ('nb') and random forest classifier ('rf')
...
from rfi.src.rfi.samplers.ensemble import UnivRFSampler
from rfi.src.rfi.decorrelators.naive import NaiveDecorrelator
import time
import argparse
...
parser.add_argument(
    ...
    type=str,
    default="nb",
    help="linear model ('nb') or random forest classifier ('rf'); default: 'nb'")
...
parser.add_argument(
    ...
    type=int,
    default=100,
    help="Custom sample size to slice df to, default: 100",
)
...
parser.add_argument(
    ...
    type=int,
    default=20,
    help="Number of orderings in SAGE algorithm; default : 20",
)
...
def main(args):
    ...
    if args.folder is None:
        ...
        create_folder(f"results/{args.folder}")
        savepath = f"results/{args.folder}"
    ...
    if args.model == "nb":
        ...
        model_details.loc[len(model_details)] = [args.data, "naive Bayes", target, acc]
        model_details.to_csv(
            f"{savepath}/model_details_{args.data}_nb.csv", index=False
        )
    ...
    # SAGE explainer
    wrk = Explainer(model_predict, fsoi, X_train, loss=log_loss, sampler=sampler,
